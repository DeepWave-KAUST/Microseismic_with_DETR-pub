...
def train(model: nn.Module,
		  ...
		  batch_size: int, nsr: float, mean, std,
		  device: torch.device):
	"""
	...
	for count in range(0, n_data, batch_size):
		...
		minibatch_data = (data[indices] + nsr * torch.randn(data[indices].size())).unsqueeze(dim=1).to(device)
		...
		# and minibatch labels for location of size (batch_size, n1_label, 2).
		minibatch_label_cls = label_cls[indices].unsqueeze(dim=2).to(device)
		minibatch_label_loc = label_loc[indices].view(-1, n1_label, 2).to(device)
		...
		# For special cases of zero-event inputs:
